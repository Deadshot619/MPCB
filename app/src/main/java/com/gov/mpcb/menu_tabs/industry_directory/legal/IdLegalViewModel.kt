...
class IdLegalViewModel : BaseViewModel<IdLegalNavigator>() {
    ...
    //Variable to hold [IdLegalData] data
    private val data = MutableLiveData<List<IdLegalData>>()
    val _data: LiveData<List<IdLegalData>>
        get() = data
    ...
    fun getIndustryData(industryId: Int, industryDirectoryType: IndustryDirectoryType){
        ...
        mDisposable.add(
            DataProvider.getApplicationListData(
                request = request,
                success = Consumer {
                    data.value = Gson().fromJson<List<IdLegalData>>(it.data.toString())
